...
Voyagent is a multi-agent AI travel planner. Interest, Budget and Logistics agents propose and challenge each other's picks, a Planner settles the final day-by-day itinerary, and the whole negotiation streams live next to a map that fills in as places are agreed. Success means a user trusts the produced itinerary enough to actually take it with them (print/PDF), and a demo viewer immediately understands that multiple agents are negotiating, not one chatbot answering.

## Positioning

Watch your trip being negotiated — the process is the proof of the plan.

## Brand Personality

Precise, warm, confident. The voice of a well-run travel bureau: it states facts (times, costs, distances) plainly and lets the itinerary speak. Emotionally the product should feel like holding a beautifully typeset route map — anticipation without noise.
...
WCAG AA floor: body text ≥4.5:1 (≥7:1 targeted for primary text), full keyboard focus states, `prefers-reduced-motion` honored on every animation. UI ships in English and Azerbaijani; layouts must tolerate both languages' string lengths.
